 window.SDK_OPTIONS = {
    gameId: "e5xg3o4qd905tb5sc2xr1phsuw2baq9n",
    onEvent: function (a) {
       switch (a.name) {
          case "SDK_GAME_PAUSE":
...
const wonSFX = new Audio("sfx/win.mp3");
const lossSFX = new Audio("sfx/lose.mp3");
const tieSFX = new Audio("sfx/tie.mp3");
const buttonSFX = new Audio("sfx/button.mp3");
const dropSFX = new Audio("sfx/drop.mp3");
...
function startGame() {
  ...
  if ($('input[name=mus-options]:checked').val() == 1) {
    const volumeFull = 1;
    bgm.volume = volumeFull;
    wonSFX.volume = volumeFull;
    lossSFX.volume = volumeFull;
    tieSFX.volume = volumeFull;
    buttonSFX.volume = volumeFull;
    dropSFX.volume = volumeFull;
  } else {
    const volumeMute = 0;
    bgm.volume = volumeMute;
    wonSFX.volume = volumeMute;
    lossSFX.volume = volumeMute;
    tieSFX.volume = volumeMute;
    buttonSFX.volume = volumeMute;
    dropSFX.volume = volumeMute;
  }

  boardVal = $('input[name=bor-options]:checked').val()
  $('.board').css("background-image", "url(images/boards/board" + boardVal + ".png)");
  ...
  if ($('input[name=dif-options]:checked').val() <= 3) {
