    ...
    window.RuntimeInterface = class RuntimeInterface {
        constructor(opts) {
            ...
            if (this._exportType === "playable-ad" ||
                this._exportType === "instant-games") this._useWorker = false;
        ...
        async _InitDOM(opts, port2) {
            ...
            for (let [originalUrl, loadUrl] of allProjectScripts) {
                ...
                } else if (originalUrl === "scriptsInEvents.js" ||
...
'use strict'; {
    ...
    const HANDLER_CLASS = class RuntimeDOMHandler extends self.DOMHandler {
        constructor(iRuntime) {
            ...
            iRuntime.AddRuntimeComponentMessageHandler("canvas",
                "update-size", e => this._OnUpdateCanvasSize(e));
            ...
            const allowDefaultContextMenuTagNames =
                new Set(["input", "textarea", "datalist"]);
        ...
        Attach() {
            ...
            if (typeof PointerEvent !== "undefined") {
                ...
                window.addEventListener("pointercancel",
                    e => this._OnPointerEvent("pointercancel", e))
            } else {
                window.addEventListener("mousedown", e => {
                    this._HandlePointerDownFocus(e);
                ...
                window.addEventListener("touchmove", e => this._OnTouchEvent("pointermove",
                    e));
            ...
            if (this._iRuntime.IsAndroid() && !this._iRuntime.IsAndroidWebView() &&
        ...
        _ScheduleSimulatedResize(width, height, count) {
            if (this._simulatedResizeTimerId !== -1) clearTimeout(this._simulatedResizeTimerId);
            this._simulatedResizeTimerId = setTimeout(() => this._OnSimulatedResize(width, height,
                count), 48)
        }
